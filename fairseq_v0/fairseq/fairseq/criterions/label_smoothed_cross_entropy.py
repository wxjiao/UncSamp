...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        if not model.training:
            ...
            # token acc
            _argmax = torch.argmax(_lprobs, dim=-1, keepdims=True)  # BxLx1
            target_acc = torch.eq(_target, _argmax).to(dtype=mtarget_lprob.dtype)  # BxLx1
            mtarget_acc = target_acc.masked_fill(_pad_mask, 2.0)
            # generate list
            #print(sample['id'].size(), mtarget_lprob.size(), mtarget_acc.size())
            #print(sample['id'].squeeze().size(), mtarget_lprob.squeeze(-1).size(), mtarget_acc.squeeze(-1).size())
            # Reduce the batchsize if zip error occurs
            sample_lprob_list = mtarget_lprob.squeeze(-1).tolist()
            sample_acc_list = mtarget_acc.squeeze(-1).tolist()
